...
description: Generates .drawio XML and exports to PNG/SVG/PDF/JPG via the native draw.io desktop CLI. Load only when the user explicitly asks to use drawio-skill, /drawio-skill, or draw.io for diagrams (flowcharts, architecture, ER/UML, network topology, mind maps, etc.). Do not auto-invoke from generic diagram or visualization requests.
license: MIT
