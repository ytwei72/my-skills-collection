...
description: 把 HTML（本地文件或 http(s) URL）转换成 PDF。默认 continuous 连续单页（保留桌面布局、不分页、文本可选）；仅当用户明确要求标准分页/A4/页码/多页打印时才用 paged。另支持页眉页脚、书签、水印、元数据、加密、合并、脱敏与预览自检。用户说"HTML转PDF""网页转PDF""单页/不分页/长图式PDF"或抱怨导出被分页/裁切时用本技能；禁止因 HTML 有章节/表格/目录就擅自改用 paged。
...
| 「HTML 里有目录/大纲 → 需要 `--toc` → 用 paged」 | `--toc` 可挂在 continuous 上；**不得**因此改 paged |
...
- `--header / --footer`：模板支持 `{title}{page}{pages}{date}` 占位符与
  `左|中|右` 三段对齐。
- `--toc [--toc-depth N]`：按 HTML 标题 `h1..hN` 生成书签大纲。
...
4. 用了 `--toc` 在阅读器里展开书签确认层级；若某标题缺失，多半是它被栅格化导致
   文本层无法定位。
