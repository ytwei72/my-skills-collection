...
把 **HTML 或 Markdown**（本地文件或 `http(s)` URL）**全功能**转换成 PDF：源为 MD 时
先经 **`md-to-rich-html`** 生成同名 `.html`（默认带文内目录），再转 PDF（默认
`--bookmarks`）。用户明确说不要目录/书签时，HTML 与 PDF 均不生成目录及书签。连续单页
与标准分页双模式，外加页眉页脚页码、书签、PDF 目录页、水印、元数据、加密、合并与预览自检。
...
| **PDF 书签**（`--bookmarks`） | 按 HTML 标题写阅读器侧栏大纲；**不改正文**、不插目录页；**默认开启**（用户说不要目录/书签时关闭） |
...
## Markdown 输入（前置 md-to-rich-html）

源文件为 `.md` / `.markdown` 时，**必须先**调用 `md-to-rich-html` 生成富 HTML，再执行
本工具。默认：HTML 含 `nav.toc` 文内目录，PDF 加 `--bookmarks`。用户说不要目录或书签时，
HTML 不生成目录块，PDF 不加 `--bookmarks` / `--toc`。

```powershell
# 1. 按 md-to-rich-html 技能生成 report.html（默认含 nav.toc）
# 2. 转 PDF（默认 continuous + 书签）
.venv\Scripts\python.exe .cursor\skills\html-pdf-studio\scripts\html2pdf.py `
    report.html -o report.pdf --width 1280 --bookmarks --toc-depth 3 --preview
```
...
.venv\Scripts\python.exe .cursor\skills\html-pdf-studio\scripts\html2pdf.py `
    ...
    --width 1280 --bookmarks --toc-depth 3 --preview
```
...
| 用户意图 | 应选模式 | 导航参数 |
|---|---|---|
| 「MD/HTML 转 PDF」「导出 PDF」等，**未说分页** | **continuous** | 默认 **`--bookmarks`** |
| 「单页 / 不分页 / 长图式 / 保留网页布局」 | **continuous** | 默认 **`--bookmarks`** |
| 「不要目录 / 不要书签 / 无目录 / 无书签」 | 保持 continuous/paged | **不加** `--bookmarks` / `--toc`；MD 前置 HTML 也不生成 `nav.toc` |
| 「加书签 / 大纲 / 侧栏导航」 | 保持用户指定的 continuous/paged | **`--bookmarks`** |
| 「PDF 目录 / 添加目录 / 目录页」 | 通常 **paged**（先确认） | **`--toc`** |
| 「A4 / 标准分页 / 多页 / 页码」 | **paged** | 按需 `--bookmarks` 或 `--toc` |
| HTML 有章节、表格、文内目录，但未要求分页 | **continuous** | 默认 **`--bookmarks`** |
| 源为 **`.md` / `.markdown`** | 先 **md-to-rich-html** → HTML | 再按上表转 PDF |
