...
4. **定信息架构**：封面式 `header`（可选）+ 按 `h1/h2` 分 `section`；长文考虑页内锚点或目录块。
   - **被 `html-pdf-studio` 调用、且用户未说不要目录/书签时**：在 `main` 开头插入
     **`nav.toc`**（`aria-label="目录"`），列出 `h1..hN` 锚点链接；样式与仓库成品
     一致（可参考 `Miscellaneous/警用笔记本电脑/证书灌装/警用笔记本证书灌装需求分析.html`）。
   - 用户明确不要目录/书签，或仅要 HTML 不要 PDF 且说无目录：不插入 `nav.toc`。
...
- 产出 HTML 将作为 **`html-pdf-studio`** 输入时：默认生成 **`nav.toc`**；用户说不要目录/书签时不生成。
